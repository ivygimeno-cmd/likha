...
export default async function MarketplacePage({
  ...
  const requestedCategory = params.category ?? "Lahat";
          ...
          <div className="mt-4 flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-lg leading-8 text-[#173d32]/70">
                Maghanap ng Filipino creators para sa custom apparel,
                personalized gifts, souvenirs at branded business materials.
              </p>
            </div>
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-3 text-[#173d32]/60">
            Tingnan ang mga open requests at mag-submit ng proposal.
          </p>
        </div>

        <p className="text-sm font-semibold text-[#173d32]/55">
          {openRequests?.length ?? 0}{" "}
          {(openRequests?.length ?? 0) === 1
            ? "request"
            : "requests"}
        </p>
      </div>
      ...
      {!openRequests || openRequests.length === 0 ? (
        <div className="mt-8 rounded-2xl border border-[#173d32]/15 bg-[#fbf8f1] px-6 py-12 text-center">
          <p className="font-serif text-2xl font-semibold">
            Wala pang open buyer requests.
          </p>

          <p className="mt-2 text-sm text-[#173d32]/55">
            Bumalik ulit kapag may bagong project na available.
          </p>
        </div>
      ) : (
              ...
              <article key={product.name} className="group">
                ...
                <div className="pt-5">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span className="shrink-0 text-sm">★ 4.9</span>
                  </div>
